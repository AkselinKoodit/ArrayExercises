...
while (i >= 0) {
  reversedArray.push(fourthArray[i]);
  i--;
}
...
fourthArray.forEach((el) => {
  fourthReversed.unshift(el);
});
console.log(fourthReversed);
...
let cleanedFifth = [];
let n = fifthArray.length - 1;
while (n >= 0) {
  if (cleanedFifth.includes(fifthArray[n])) {
    n--;
    continue;
  } else {
    cleanedFifth.push(fifthArray[n]);
    n--;
  }
}
console.log(cleanedFifth);
...
let i2 = sixthArray.length - 1;
let message = `Let's compare arrays!`;
while (i2 >= 0) {
  if (sixthArray.includes(seventhArray[i2])) {
    message = `The arrays have same data`;
  } else {
    message = `The arrays don't have the same data`;
  }
  i2--;
}
console.log(message); //This solution doesn't work if the second array is larger-->you could compare lengths first (if they are not the same, they don't have the same data)
...
let withoutArrays = tenthArray.flat(4);
...
names = peopleInfo.filter((el) => {
  if (phones.includes(el) == false && emails.includes(el) == false) {
    return el;
  }
});
console.log(names);
